"""Run the German BESS optimization model over multiple input days and EPR values.
...
import GER_BESS_optimizer


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run German BESS MILP cases.")
    parser.add_argument("--input-dir", default="Data/Germany/input_data", help="Directory containing Pyomo .dat input files.")
    parser.add_argument("--output-dir", default="results/GERMANY/Optimization_Results", help="Directory where optimized Pyomo instances are written as .pkl files.")
...
def main() -> None:
    ...
    for input_file in input_files:
        ...
        print(f"Loading German input data for {date_token}: {input_file}")

        for epr in args.epr_values:
            data = pyo.DataPortal(model=GER_BESS_optimizer.model)
            data.load(filename=str(input_file))
            data.data()["EPR"] = {None: epr}

            instance = GER_BESS_optimizer.model.create_instance(data)
            ...
            output_file = output_dir / f"GER_opt_results_{date_token}_EPR{epr_label}.pkl"
